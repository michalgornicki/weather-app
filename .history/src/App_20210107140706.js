...
class App extends React.Component {
  ...
  get_WeatherIcon (icons, rangeId) {
    switch (true) {
      ...
      case rangeId >= 801 && rangeId <= 804:
        this.setState({icon: this.weatherIcon.Clouds, background: this.weatherBackground.Atmosphere});
        break;
        default:
  ...
  getWeather = async () => {
    const api_call = await fetch(
      `http://api.openweathermap.org/data/2.5/weather?q=New York&appid=${API_key}`
      );
