...
class App extends React.Component {
  ...
  getWeather = async (e) => {
    ...
    if (response.city) {
      this.setState({
        ...
        visibility:
          "visibility: " + response.list[0].visibility + " meters",
        ...
        celsius_aftertomorrow: this.calCelsius(
          response.list[16].main.temp
        ),
        wind_aftertomorrow: this.calWind(
          response.list[16].wind.speed
        ),
        description_aftertomorrow:
          response.list[16].weather[0].description,
        celsius_afteraftertomorrow: this.calCelsius(
          response.list[24].main.temp
        ),
        wind_afteraftertomorrow: this.calWind(
          response.list[24].wind.speed
        ),
        description_afteraftertomorrow:
          response.list[24].weather[0].description,
        tomorrow: "Tomorrow: ",
        aftertomorrow: "Tomorrow +1: ",
        afteraftertomorrow: "Tomorrow +2: ",
        error: false,
      });

      this.get_WeatherIcon(
        this.weatherIcon,
        response.list[0].weather[0].id
      );
    } else {
      this.setState({
        error: true,
  ...
  dayOfWeek = () => {
    ...
    var arrayOfWeekdays = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"];
    var today = arrayOfWeekdays[dayNumber];
    console.log(today);
  }
      ...
      <div className={`App ${this.state.background}`}>
        <div className="wrap-app">
          ...
          <Weather
            ...
            temp_celsius_aftertomorrow={
              this.state.celsius_aftertomorrow
            }
            wind_aftertomorrow={this.state.wind_aftertomorrow}
            description_aftertomorrow={
              this.state.description_aftertomorrow
            }
            temp_celsius_afteraftertomorrow={
              this.state.celsius_afteraftertomorrow
            }
            wind_afteraftertomorrow={
              this.state.wind_afteraftertomorrow
            }
            description_afteraftertomorrow={
